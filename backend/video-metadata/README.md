# PlayGround - Video Matadata API server

## 사용법

- PROTOCOL : http
- HOST : 27.96.135.160
- PORT : 3000

\* 추후 Nginx 프록시를 사용하면 아래 코드의 BASE 파트(ip, port)는 수정될 것입니다.

### 1. 전체 영상 정보 호출

- GET
- parameter : X
- return : array of videos

```js
let data;
fetch("http://27.96.135.160:3000/videos")
  .then((res) => res.json())
  .then((res) => (data = res));
console.log(JSON.stringify(data, null, 2));

/* 출력 결과 :
[
  ({
    _id: "5ec25a43dd4cfe88ab3edab7",
    name: "영화 왓수다 제1회",
    uploadedAt: "2020-04-24T00:00:00.000Z",
    summary: "MBC every1 최초 영화 전문 프로그램",
    url: "http://49.50.162.195:8080/videos/whatsuda.m3u8",
  },
  {
    _id: "5ec25a43dd4cfe88ab3edab8",
    name: "뭉찬외전 - 감독님이 보고 계셔 오싹한 과외 제1회",
    uploadedAt: "2020-04-26T00:00:00.000Z",
    summary: "어쩌다FC 전설들이 10세 수준의 축구 실력을 탈출하기 위해 '오싹한' 심화 보충수업을 받는 이야기",
    url: "http://49.50.162.195:8080/videos/mungchan.m3u8",
  },
  {
    _id: "5ec25a43dd4cfe88ab3edab9",
    name: "아버지의 초상",
    uploadedAt: "2016-01-28T00:00:00.000Z",
    summary:
      "회사의 부당한 구조조정으로 인해 한 순간에 실업자가 된 '티에리'.\n    그는 사랑하는 아내와 아들을 지켜야 하는 한 집안의 가장이다.\n    2년 간의 구직활동 끝에 그는 대형마트의 보안요원으로 취직하게 되고,\n    매장 내 감시카메라를 통해 모든 사람을 감시하는 업무를 맡게 된다.\n    그러나 일자리를 얻은 기쁨도...",
    url: "http://49.50.162.195:8080/videos/father.m3u8",
  },
  {
    _id: "5ec25a43dd4cfe88ab3edaba",
    name: "하트비트",
    uploadedAt: "2010-11-25T00:00:00.000Z",
    summary:
      "마리와 그녀의 절친한 친구이자 게이인 프랑시스. 영리하고 날카로운 마리와 다정하고 섬세한 프랑시스는 서로를 보완하는 좋은 친구사이다. 취미는 달라도 취향은 같은 두 친구는 늘 함께 어울리며 서로를 아끼며 지켜주는 든든한 버팀목이 되어왔다. 어느 날, 두 사람은 친구들과 어울린 파티에서 아름다운 니콜라를 만나 둘 다 첫눈에 반한다. 만남이 거듭될수록 운명적 사랑이라 느껴지는 그가 보내오는 셀 수 없는 무수한 사랑과 관심의 신호들에 설레며, 프랑시스와 마리는 점점 더 깊이 니콜라에게 빠져든다. 마리와 프랑시스는 서로의 우정이 변할 수 있을 거라고는 단 한번도 의심해 본 적이 없지만 아도니스를 닮은 매력적인 니콜라 앞에서, 두 사람 모두 사랑 앞에서 한치도 물러설 수 없는 라이벌이 된다. 곧 두 친구는 자신들이 결코 깨지지 않을 거라 믿었던 우정이 이 사랑의 경쟁 앞에서 무너질 위기에 처했다는 것을 알게 되는데….",
    url: "http://49.50.162.195:8080/videos/heartbeat.m3u8",
  },
  {
    _id: "5ec25a43dd4cfe88ab3edabb",
    name: "<강(江),원래> 초이스 2 :: 자전거의 이름으로(무료)",
    uploadedAt: "2020-01-02T00:00:00.000Z",
    summary:
      "다큐멘터리 감독과 미디어 활동가들이 카메라를 들고 한강, 낙동강, 금강, 영산강, 섬진강을 두루 다니며, 헤아릴 수 없이 시간을 흘러온 강과 그 강과 함께 살아온 생명들, 그리고 사람들의 나지막한 목소리에 귀 기울인다. \"강(江), 원래\"는 '강을 원래대로 두라!'는 사람들의 절박한 호소이다.\n      섬진강 시멘트 도로의 비포 앤 에프터 사진이 트위터를 통해 확산되고, 우리는 자전거 생활자인 지음, 살구와 함께 섬진강에 내려간다.",
    url: "http://49.50.162.195:8080/videos/river.m3u8",
  },
  {
    _id: "5ec25a43dd4cfe88ab3edabc",
    name: "또한번 엔딩 특별회",
    uploadedAt: "2020-03-21T00:00:00.000Z",
    summary: "'신혼부부 전세 대출'을 받기 위해 결혼을 결심한 비혼주의자들의 아찔한 결혼 사기 로맨스",
    url: "http://49.50.162.195:8080/videos/ending.m3u8",
  })
]
*/
```

### 2. 개별 영상 정보 호출

- GET
- parameter : id (string)
- return : single video object

```js
let data;
fetch("http://27.96.135.160:3000/video/5ec25a43dd4cfe88ab3edabc")
  .then((res) => res.json())
  .then((res) => (data = res));
console.log(JSON.stringify(data, null, 2));
/*
{
  "_id": "5ec25a43dd4cfe88ab3edabc",
  "name": "또한번 엔딩 특별회",
  "uploadedAt": "2020-03-21T00:00:00.000Z",
  "summary": "'신혼부부 전세 대출'을 받기 위해 결혼을 결심한 비혼주의자들의 아찔한 결혼 사기 로맨스",
  "url": "http://49.50.162.195:8080/videos/ending.m3u8"
}
*/
```
